...
`nc` сообщает нам, что на порту 80 работает HTTP (веб-сервер), но баннер не отображается.
